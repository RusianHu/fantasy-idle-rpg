...
| Hazard 特效实验室 | `hazards/hazards.html?seed=1234ABCD&region=grassland&lang=zh-CN` | 直接挂载正式 Hazard 实例、固定 roll 概率侦测与 50ms 状态机；显示策略、远征视野、天气模拟倍率和来源诊断，以真实检定审计提前识破、未识破预警与逃生，并用六阶段对照板检查九类像素机关的材质、范围、方向、碎屑、残留、事件文案与降动态表现 |
...
| 天气 / 气候筹备型渲染 Lab | `weather-climate/weather-climate.html?seed=1234ABCD&region=forest&time=300&particle=region&lang=zh-CN` | 只编排正式 `terrain v3 → renderer → daynight → particles → effects` 链路，集中对照八区天空、昼夜、视差、光柱、粒子原语、风采样与地形反馈；不实现天气系统，报告中的时间线、天气状态、强度、可见度 Provider 与渲染层未来槽位固定为 `unconnected` |

## 维护约束

- 优先加载正式注册表、系统和渲染模块；不得复制一套脱离生产代码的模拟实现。
- 正式入口与五个工作台只加载一个当前 `BUILD_ID` 的 `js/data/content/content.generated.js`；不得直接加载 `*.pack.js`、`*.support.js` 或内容侧 `js/data/content/manifest.generated.js`。生产使用的探索资产 manifest 不受此限制。
...
`tests/browser-smoke.js` 在移动与桌面视口验证正式入口和五个工作台的非空 Canvas、无横向溢出、44px 控件、双语、中立 Observe/Attack 二次确认与固定 tick Engagement 闭环；天气 / 气候 Lab 另验证 URL 恢复、四阶段截图、只读 QA API、确定性与零存档副作用。Hazard 单元测试覆盖固定 roll、概率成功/失败、策略/视野/环境倍率、隐藏触发、导航代价、Provider 降级、聚合分布、九类视觉符号、两层渲染与独立 Lab 生产入口。`tests/map-effects-inspector.test.js` 保证现场页逐类枚举正式 Region/Population/MountPlan/SpawnLease，且全部检查器键具备中英文。`tests/action-bubble-demo.test.js` 在 Lab 验证 53 个 Actor，以及由 16 个正式 Encounter 加 2 个 QA Encounter 组成的 18 个目录条目，并覆盖接敌不重叠、正式攻击 FX 与 PresentationEvent；`tests/unit-ecosystem-v14.test.js` 覆盖稳定生成、放置与重生、Relation/Engagement 原子性、Objective、奖励授权、Variant 与 v14 社交边界，其余确定性、4+8 性能与平衡矩阵由 `tests/v2-*.test.js` 覆盖。
