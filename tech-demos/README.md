...
- 正式入口与三个工作台只加载一个当前 `BUILD_ID` 的 `js/data/content/content.generated.js`；不得直接加载 `*.pack.js`、`*.support.js` 或生成 manifest。
...
`tests/browser-smoke.js` 在移动与桌面视口验证正式入口和三个工作台的非空 Canvas、无横向溢出、44px 控件、双语、中立 Observe/Attack 二次确认与固定 tick Engagement 闭环；`tests/map-effects-inspector.test.js` 保证现场页逐类枚举正式 Region/Population/MountPlan/SpawnLease，且全部检查器键具备中英文。`tests/action-bubble-demo.test.js` 在 Lab 验证 29 个 Actor、18 个 Encounter、接敌不重叠、正式攻击 FX 与 PresentationEvent；`tests/unit-ecosystem-v14.test.js` 覆盖稳定生成、放置与重生、Relation/Engagement 原子性、Objective、奖励授权、Variant 与 v14 社交边界，其余确定性、4+8 性能与平衡矩阵由 `tests/v2-*.test.js` 覆盖。生成器的 1,600 布局与 5,000 拓扑模糊种子由 `tests/v1_13.test.js` 覆盖，八区 384 次双向营地—巢穴行程、已知循环种子与中断语义由 `tests/navigation-long-route.test.js` 覆盖。
