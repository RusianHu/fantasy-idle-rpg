...
`tests/browser-smoke.js` 在移动与桌面视口验证正式入口和五个工作台的非空 Canvas、无横向溢出、44px 控件、双语、中立 Observe/Attack 二次确认与固定 tick Engagement 闭环；天气 Lab 另验证 URL 恢复、四阶段截图、八区气候、强制雷暴、确定性、性能诊断与零存档副作用。`tests/weather-climate-system.test.js` 覆盖内容绑定、天气锋/过渡、跨区微气候、雷击序列、Hazard Provider 和 v16 存档不变量。
