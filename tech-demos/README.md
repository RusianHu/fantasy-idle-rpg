...
| 生成器审计 | `exploration-v3/exploration-v3.html?seed=20260727&region=grassland&scenario=gather-fallback&policy=compare&lang=en` | 复用编译 Region 的宏观拓扑、硬阻挡、营地—巢穴真实分段足迹、航点/耗时/恢复诊断、内容分布与 5×3 区块；以生产 AI 移动判定和正式导航对照采集/宝箱/接敌/过期中断及失败缓存，提供事件日志、足迹、修复前基线/修复后生产逻辑和 32 种子批量评估 |
